...
def main(args, config):
    ...
    globals()[package_manager + "_update"](installed_name)
    okay(f"Successfully updated '{package_name}'")

def apt_update(package):
	package_manager = "apt"
	command = ["sudo", package_manager, "install", "--only-upgrade", package]
	run(command, package_manager, package)

def pacman_update(package):
	package_manager = "pacman"
	command = ["sudo", package_manager, "-Syu", package]
	run(command, package_manager, package)

def pipx_update(package):
	package_manager = "pipx"
	command = [package_manager, "upgrade", package]
	run(command, package_manager, package)

def yay_update(package):
	package_manager = "yay"
	command = [package_manager, "-Syu", package]
	run(command, package_manager, package)


def snap_update(package):
	package_manager = "snap"
	command = ["sudo", package_manager, "refresh", package]
	run(command, package_manager, package)
...
def run(command, package_manager, package):
	...
	while True:
		try:
			...
			error(f"There was an error updateing '{package}' with {package_manager}! Here is the {package_manager} error:")
			print(e.stderr)
			break
...
def confirm(package, package_manager):
	res = get(f"Are you sure you want to update '{package}' with {package_manager}? [y/n]: ")
	if res != "y":
		error(f"Aborting")
		exit()